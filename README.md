...
**Alpha 4.7.0 — los 4 Niveles del plan original cerrados a alcance completo, confirmado en hardware real** (Dell Latitude 5490):
...
- **Nivel 2 (arranque y login):** Plymouth con marca Solwed, fondo y CSS de GDM en amarillo Solwed (no solo el fondo), **tema gráfico de GRUB propio** (fondo, tipografía, timeout de 5s, Solwed OS por defecto), idioma "Spanish" resaltado por defecto en el menú de arranque del propio ISO, usuario/hostname live, iconos de ArcMenu/apariencia rebrandeados, teclado español, tema de GNOME Shell y GTK en amarillo de acento.
- **Nivel 3 (comportamiento y apps):** LibreOffice, Thunderbird (PPA de mozillateam, `.deb` real, con plugin de WhatsApp Web preinstalado), Brave como navegador por defecto, Autofirma + Okular (firma electrónica), Timeshift, instalador gráfico de FacturaScripts bajo demanda, menú de inicio (ArcMenu) curado con carpeta LibreOffice y anclados propios. Icono de Software/FacturaScripts recoloreado azul→naranja.
- **Nivel 4 (avanzado), cerrado del todo:** RustDesk autoalojado (servidor `hbbs`/`hbbr` propio en `erpsolwed`, DNS real + zero-touch confirmado), repositorio APT propio (`aptly`, DNS real + certificado Let's Encrypt real), y asistente de bienvenida (diálogo de un solo uso en el primer login).
- **Slideshow del instalador (Ubiquity):** reescrito por completo en los 27 idiomas soportados — contenido genérico de Ubuntu/AnduinOS (juegos, Docker, Flatpak) sustituido por contenido propio y relevante (catálogo de apps + Timeshift, soporte remoto con RustDesk, actualizaciones), con capturas reales del sistema instalado (no genéricas) y el wallpaper de marca sin el logo superpuesto.
- **Investigación aparcada, pendiente de decisión de negocio:** RDP multiusuario simultáneo (`gnome-remote-desktop` en modo `--system`/headless, ya instalado de fábrica) con una cuenta por técnico en vez de la compartida `soporte-solwed` — pendiente de que el usuario hable con su responsable sobre el modelo de acceso de red (VPN vs. IP pública).
